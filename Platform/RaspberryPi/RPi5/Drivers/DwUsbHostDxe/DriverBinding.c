  ...
  )
{
  ...
  if ((Mode != UsbDrModeHost) && (Mode != UsbDrModeOtg)) {
    DEBUG ((DEBUG_INFO, "DwUsbHostDxe: dr_mode=%a, skipping host driver load\n", ModeStr));
    return EFI_UNSUPPORTED;
  }

  DEBUG ((DEBUG_INFO, "DwUsbHostDxe: dr_mode=%a, loading host driver\n", ModeStr));
